...
printf "│ IPv4: %-22s │ 运行: %-25s │\n" "${ipv4:-未分配/禁用}" "${up_time:-未知}"
echo -e "└────────────────────────────────────────────────────────────┘"
...
if $(awk 'BEGIN{print ('$load_1m' > '$cpu_cores')?1:0}'); then
...
echo -e "  $((adv_idx++)). ${C_WHITE}[权限隔离]${C_RESET} 鉴于当前为 SMM 核心业务，强烈建议分离前端与后端的用户运行组，切忌使用 root 权限强跑 Node。"
